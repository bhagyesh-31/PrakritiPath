 // Firebase Configuration
        ...
        snapshot.forEach(doc => {
          ...
          card.innerHTML = `
            <div class="contribution-image" style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.1), rgba(59, 130, 246, 0.1)); border-bottom: 2px solid var(--border);">
              ${contrib.imageUrl ? `<a href="${contrib.imageUrl}" target="_blank" style="width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; text-decoration: none;"><img src="${contrib.imageUrl}" style="width: 100%; height: 100%; object-fit: cover; display: block;" onerror="this.parentElement.innerHTML='<div style=\"display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; color: var(--text-muted); font-size: 1.5rem;\">📸 View on Drive</div>'"></a>` : '<div style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; font-size: 2rem;">📸</div>'}
            </div>
            <div class="contribution-content">
              ...
              <div style="margin-top: 10px;">
                ...
                <span style="background: ${contrib.verified ? 'rgba(34, 197, 94, 0.2)' : 'rgba(245, 158, 11, 0.2)'}; color: ${contrib.verified ? 'var(--success-green)' : 'var(--warning-yellow)'}; padding: 4px 8px; border-radius: 4px; font-size: 0.8rem; margin-left: 5px;">${contrib.verified ? '✅ Verified' : '⏳ Pending'}</span>
              </div>
            </div>
          `;
      ...
      db.collection('contributions')
        ...
        .then(snapshot => {
          ...
          snapshot.forEach(doc => {
            ...
            card.innerHTML = `
              <div class="contribution-image" style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.1), rgba(59, 130, 246, 0.1)); border-bottom: 2px solid var(--border);">
                ${contrib.imageUrl ? `<img src="${contrib.imageUrl}" style="width: 100%; height: 100%; object-fit: cover; display: block;" onerror="this.parentElement.innerHTML='<div style=&quot;display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; color: var(--text-muted);&quot;>📸 View on Drive</div>'">` : '<div style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; font-size: 2rem;">📸</div>'}
              </div>
              <div class="contribution-content">
                <div class="contribution-type">${contrib.actionType}</div>
                <p class="contribution-caption">${contrib.caption}</p>
      ...
      .then(snapshot => {
        ...
        snapshot.forEach(doc => {
          ...
          card.innerHTML = `
            ...
            ${contrib.imageUrl ? `
              <div style="background: linear-gradient(135deg, rgba(16, 185, 129, 0.1), rgba(59, 130, 246, 0.1)); border-radius: 12px; overflow: hidden; margin-bottom: 15px; border: 1px solid var(--border); box-shadow: 0 8px 24px rgba(16, 185, 129, 0.15);">
                <img src="${contrib.imageUrl}" style="width: 100%; height: 300px; object-fit: cover; display: block;" onerror="this.parentElement.innerHTML='<div style=\"display: flex; align-items: center; justify-content: center; height: 300px; background: rgba(16, 185, 129, 0.1); color: var(--text-muted);\">📸 Image not available</div>'">
              </div>
            ` : ''}
  ...
  function updateLikeStatus(postId) {
    db.collection('likes').where('postId', '==', postId).get().then(snapshot => {
      ...
      document.getElementById('likeCount-' + postId).textContent = likeCount + (likeCount === 1 ? ' like' : ' likes');
  ...
  function showComments(postId) {
    ...
    db.collection('comments').where('postId', '==', postId).orderBy('createdAt', 'asc').get().then(snapshot => {
      ...
      html += `<div style="margin-top: 12px; display: flex; gap: 8px;"><input type="text" placeholder="Share your thoughts..." id="commentInput-${postId}" style="margin-bottom: 0; flex: 1;"><button class="btn btn-primary" style="padding: 8px 16px; white-space: nowrap;" onclick="addCommentHandler('${postId}')">💬 Post</button></div></div>`;
  ...
  function addCommentHandler(postId) {
    ...
    db.collection('comments').add({
      ...
      // Add small delay to ensure Firestore sync
      setTimeout(() => {
        showComments(postId);
      }, 300);
    }).catch(error => {
